...
func (s *Service) HandleTask(task RobotTask) {
	...
	for _, cmd := range task.Commands {
		...
		// Make sure if the task is cancelled, we stop processing
		...
		s.ExecuteRobotCommand(cmd, task.DelayBetweenCommands) // Execute each command in the task
	}
...
func (s *Service) ExecuteRobotCommand(cmd RobotCommand, durationBetweenCmds CommandDuration) {
	...
	time.Sleep(time.Duration(durationBetweenCmds)) // Simulate delay between commands
